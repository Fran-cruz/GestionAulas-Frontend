...
import {
  ...
  listPeriodos,
  activatePeriodo,
} from '../periodosServices/PeriodosServices';
...
export function Periodos() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try {
      await createPeriodo({ ...form, nombre: form.nombre.trim(), estado: 'CERRADO' }, authUserId);
      setForm(emptyForm);
      setSuccessMessage('Período académico creado correctamente.');
      await loadPeriodos();
    } catch (error) {
      setFormError(error instanceof ApiError ? error.message : 'No se pudo guardar el período.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleSelectPeriodo = async (periodo: PeriodoAcademico) => {
    ...
    try {
      ...
      setActionError(error instanceof ApiError ? error.message : 'No se pudo activar el período.');
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const handleDeletePeriodo = async (periodo: PeriodoAcademico) => {
    ...
    try {
      ...
      setActionError(error instanceof ApiError ? error.message : 'No se pudo borrar el período.');
    } finally {
      setDeletingId(null);
    }
  };
      ...
      <div className="content-grid" style={{ gridTemplateColumns: '0.82fr 1.18fr', paddingTop: 40 }}>
        <section className="table-card" style={{ padding: 28, minHeight: 560 }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 28 }}>
            ...
            <button type="submit" className="primary-btn" disabled={saving} style={{ marginTop: 20, width: 270 }}>
              ...
              {loading ? (
                ...
                sortedPeriodos.map((periodo) => (
                  <tr key={periodo.id}>
                    ...
                    <td>
                      <span className={`tag state ${periodo.estado.toLowerCase()}`}>{formatEstado(periodo.estado)}</span>
                    </td>
                    <td>
